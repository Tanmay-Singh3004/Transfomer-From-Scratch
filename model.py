...
class MultiHeadAttention(nn.Module):
        def __init__(self, d_model : int, h : int, dropout : float):
                ...
                self.d_model = d_model
                self.h = h
                assert d_model % h == 0, f"embedding dimension ({d_model}) is not divisble by number of heads ({h})"
                self.d_k = d_model // h
...
class EncoderBlock(nn.Module):
        def __init__(self, self_attention : MultiHeadAttention, feed_forward : FeedForward, dropout : nn.Dropout):
                super().__init__()
...
class Transformer(nn.Module):
        def __init__(self, encoder : Encoder, decoder : Decoder, src_embed : InputEmbeddings, tgt_embed : InputEmbeddings, src_pos : PositionalEncoding, tgt_pos : PositionalEncoding, project : ProjectionLayer):
                super().__init__()
